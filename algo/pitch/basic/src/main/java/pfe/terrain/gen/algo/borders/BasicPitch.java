...
public class BasicPitch extends Contract {
    ...
    public static final SerializableKey<DoubleType> facePitchKey =
    ...
    private final static Comparator<Coord> byHeight =
            (o1, o2) ->
                    (int) (1000 * (o2.getProperty(vertexHeightKey).value - o1.getProperty(vertexHeightKey).value));

    @Override
    public void execute(IslandMap islandMap, Context context) {
        FaceSet faces = islandMap.getFaces();
        for (Face face : faces) {
            face.putProperty(facePitchKey, new DoubleType(computePitch(face)));
        }
    }

    double computePitch(Face face) {
