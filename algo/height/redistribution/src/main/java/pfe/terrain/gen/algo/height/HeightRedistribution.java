...
public class HeightRedistribution extends Contract {
    ...
    private Param<Double> factorKey = new Param<>("Height factor", Double.class, "0-1", "Dunno", 1.0);
    ...
    public void execute(IslandMap map, Context context)
        ...
        double scaleFactor = context.getParamOrDefault(factorKey);
        for (int i = 0; i < orderedVertices.size(); i++) {
            double y = i / (double) (orderedVertices.size() - 1);
            double x = Math.pow(1, 1 / scaleFactor) - Math.pow(1 - y, 1 / scaleFactor);
