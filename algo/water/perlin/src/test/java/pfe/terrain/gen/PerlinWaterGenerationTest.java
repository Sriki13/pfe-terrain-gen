...
import static pfe.terrain.gen.PerlinWaterGeneration.waterKindKey;

public class PerlinWaterGenerationTest {
    ...
    public void setUp() throws Exception {
        PerlinWaterGeneration waterGen = new PerlinWaterGeneration();
        map = new IslandMap();
        faces = new FaceSet();
        mapSize = 200;
    ...
    public void testPropertyIsthere() throws NoSuchKeyException, KeyTypeMismatch {
        faces = map.getFaces();
        for (Face face : faces) {
            assertThat(face.getProperty(PerlinWaterGeneration.faceWaterKey).value, anyOf(is(true), is(false)));
            if (face.getProperty(PerlinWaterGeneration.faceWaterKey).value) {
